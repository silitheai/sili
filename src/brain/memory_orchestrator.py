...
class MemoryOrchestrator:
    ...
    def get_relevant_context(self, query, user_id, depth=1):
        ...
        primary_memories = self.vector.semantic_search(user_id, query, n_results=3)
        ...
        if depth > 0:
            ...
            for link in associative_links[:2]:
                hop_memory = self.vector.semantic_search(user_id, link, n_results=2)
    ...
    def store_experience(self, user_id, user_msg, agent_response):
        ...
        self.vector.add_interaction(user_id, "assistant", f"Goal: {user_msg}\nResponse: {agent_response}")
